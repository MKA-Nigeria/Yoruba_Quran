...
class PagesLoader {
    ...
    private Pair<Integer, Integer> getPageColsRows() {
        float ratioX = 1f / pdfView.getOptimalPageWidth(pageNumber);
        float ratioY = 1f / pdfView.getOptimalPageHeight(pageNumber);
    ...
    private Holder getPageAndCoordsByOffset(float offset) {
        ...
        if (pdfView.isSwipeVertical()) {
            holder.page = pdfView.getPageNumberOfOffset(fixOffset);
            holder.row = MathUtils.floor(Math.abs(fixOffset - pdfView.toCurrentScale(pdfView.getOptimalPageOffset(holder.page))) / rowHeight);
            holder.col = MathUtils.floor(xOffset / colWidth);
        } else {
            holder.page = pdfView.getPageNumberOfOffset(fixOffset);
            holder.col = MathUtils.floor(Math.abs(fixOffset - pdfView.toCurrentScale(pdfView.getOptimalPageOffset(holder.page))) / colWidth);
    ...
    public void loadPages() {
        ...
        thumbnailWidth = (int) (pdfView.getOptimalPageWidth(pageNumber) * Constants.THUMBNAIL_RATIO);
        thumbnailHeight = (int) (pdfView.getOptimalPageHeight(pageNumber) * Constants.THUMBNAIL_RATIO);
        ...
        colWidth = pdfView.toCurrentScale(pdfView.getOptimalPageWidth(pageNumber)) / colsRows.first;
        rowHeight = pdfView.toCurrentScale(pdfView.getOptimalPageHeight(pageNumber)) / colsRows.second;
