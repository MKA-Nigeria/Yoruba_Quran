...
public class SimpleBitmapPool implements BitmapContainer {
    ...
    Bitmap[] bitmaps;
    PdfRendererParams paramsList[];

    private int poolSize;

    public SimpleBitmapPool(PdfRendererParams params[]) {
        Log.d("SIZE", "SIZE OF PARAAM: "+params.length);
        this.poolSize = getPoolSize(params[0].getOffScreenSize());
...
*/
    ...
    protected void createBitmapAtIndex(int index) {
        Bitmap bitmap = Bitmap.createBitmap(paramsList[index].width, paramsList[index].height, paramsList[index].config);
